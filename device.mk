...
ifeq ($(strip $(BOARD_USE_LOW_MEM)), false)
PRODUCT_COPY_FILES += \
...
PRODUCT_COPY_FILES += \
    device/rockchip/rksdk/fstab.$(TARGET_BOARD_HARDWARE).low_mem.bootmode.unknown:root/fstab.$(TARGET_BOARD_HARDWARE).bootmode.unknown \
    device/rockchip/rksdk/fstab.$(TARGET_BOARD_HARDWARE).low_mem.bootmode.emmc:root/fstab.$(TARGET_BOARD_HARDWARE).bootmode.emmc
endif
# For audio-recoard 
PRODUCT_PACKAGES += \
    libsrec_jni
...
ifeq ($(BOARD_STORAGE_HAVE_HDCP2X_KEY),true)
    PRODUCT_PROPERTY_OVERRIDES += ro.secure.hdcp2xkey.inside=true
else
    PRODUCT_PROPERTY_OVERRIDES += ro.secure.hdcp2xkey.inside=false
endif
